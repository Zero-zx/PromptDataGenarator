# Hướng dẫn chuẩn bị Dataset cho Qwen-Image-Edit LoRA Training

## Tổng quan

Dataset `vietnamese_dataset` hiện tại chứa các ảnh với text tiếng Việt được render trên nền trắng. Để train Qwen-Image-Edit LoRA, bạn cần:
...
**Lưu ý**: Phải thêm flag `--edit` cho Qwen-Image-Edit!
...
qwen_image_edit_control_resolution = [1024, 1024]  # Resize control về 1M pixels
num_repeats = 1
```
...
3. **Flag `--edit`**: Bắt buộc phải có khi:
   - Cache latents
   - Cache text encoder outputs
   - Train LoRA

4. **Resolution**: Dataset hiện tại dùng 1328x1328. Bạn có thể thay đổi trong script hoặc config.
...
### Lỗi khi cache/train
- Đảm bảo đã thêm flag `--edit`
- Kiểm tra đường dẫn model files (DiT, VAE, Text Encoder)
